...
class SimpleController(Node):
    ...
    def __init__(self):
        ...
        self.left_rpm_sub_ = self.create_subscription(Float64, "/left_wheel_rpm", self.left_callback, 10)
        self.right_rpm_sub_ = self.create_subscription(Float64, "/right_wheel_rpm", self.right_callback, 10)
    ...
    def left_callback(self,msg):
        self.get_logger().info("subsrcibed to left wheel rpm topic")
        self.left_rpm = msg
        self.left_vel = ((self.left_rpm.data * 2 *3.14)/60) #RPM to rad/s
        self.vel_cmd_publish()

    def right_callback(self, msg):
        self.get_logger().info("subsrcibed to right wheel rpm topic")
        self.right_rpm = msg
        self.right_vel = ((self.right_rpm.data * 2 *3.14)/60) #RPM to rad/s
        self.vel_cmd_publish()

    def vel_cmd_publish(self):
        if self.left_vel and self.right_vel:
            self.get_logger().info("got the wheel RPMs")
            vel_cmd = Float64MultiArray()
            vel_cmd.data = [self.left_vel, self.right_vel]
            self.wheel_cmd_pub_.publish(vel_cmd)
